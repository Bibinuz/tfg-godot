...
func _process(delta: float) -> void:
	...
	if is_placed:
		...
		meshes[0].set_instance_shader_parameter("custom_time", time_accum)
...
func place_belt() -> void:
	if GlobalScript.focused_element is Shaft or GlobalScript.focused_element is MachinePort:
		...
		if len(nodes_connected) == 0:
			...
			position = nodes_connected[0].global_position - center_position
...
func manage_belt_items(delta: float) -> void:
	...
	if trying_to_pass:
		#Segueix fallant, no ho pillo massa xd
		# Els objectes quan arriben al final de la cinta tornen al principi, pero aixo nomes si son els ultims en un seguit d'items
		# Pot ser un problema amb el trying to pass? Pero sense aixo les maquines no accepten inputs
